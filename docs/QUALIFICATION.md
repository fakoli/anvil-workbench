...
**STILL BLOCKED:** `advanced_routes` **route discovery** is now wired
(`GET /api/chat/advanced/routes`, read-only projection), but the advanced
**run/dispatch execution path** (`advanced_runtime`/`advanced_dispatch`) remains
**not wired to any HTTP endpoint** (the client degrades to 503 for a run), so the
live route-supported-vs-assumed control distinction and live-trace qualification
are **not** done. The release record must state that Advanced Chat success is **not**
