...
| --- | --- | --- |
| Workbench API and bridge contracts | `python -m pytest -q` | 49 passed; covers State packet/event isolation, an exact local verification-command allowlist with no shell, evidence-submit ordering, approval replay/diff invalidation, atomic approval-consumer and typed-operation-input rejection, atomic approval/run/lease validation, fenced/renewed/released worktree leases, session-bound GitHub worktrees, PR-to-merge lease retention, consumed-merge-only completion, PR-head compare-and-swap, task-to-run binding, action-failure reconciliation, bridge command recovery, bridge-project isolation, version-pinned workflows, voice redaction/error behavior, bridge-local skill digest binding/probing, selected-skill forwarding, router decision-summary compatibility, standard Responses output extraction, directives, and Serving-only sandbox boundaries. |
