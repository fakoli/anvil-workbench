...
- **Implemented (read-model + capture point, NOT wired into the live queue):**
  `workbench/models.py` defines the frozen `RunContext` and its typed parts. It
  serializes into two separately labeled structures — `trusted`
  (`trusted_execution_policy`: identity, version-pinned workflow/catalog/profile
  digests, gated capabilities, pinned skills, constraints, workflow cursor) and
  `untrusted` (`untrusted_task_data`: PRD/task title, acceptance criteria, scope,
  verification plan, evidence). `run_capabilities_from_snapshot` /
  `run_skills_from_snapshot` derive the gated authority and pinned skills from a
  compiled `WorkflowSnapshot`; every capability inherits the conservative default
  gate (`external_effect`/`policy_mutation` → `approval`, else `none`).
- **Persistence slice:** `workbench/run_context_store.py`
  (`MemoryRunContextStore`) captures and reads the immutable queue-time snapshot
  per `(project_id, run_id)`. Capture is immutable (a later PRD/task rename
  produces a different context that the store refuses to rewrite —
  `RunContextImmutableError`); a cross-project or missing read is the indistinct
  `UnknownRunContextError` (no existence oracle).
- **Capture-before-dispatch enforcement:** `dispatch_with_run_context`
  (`workbench/run_context_store.py`) resolves, then persists, then dispatches —
  in that strict order — so a context whose required authority/readable field
  cannot be resolved fails closed and no effect runs. This helper has **no
  production caller yet** — wiring it into the live bridge poll/queue loop is
  T006+ scope.
- **Read API:** `GET /api/projects/{project_id}/runs/{run_id}/context`
  (`build_run_context_router` in `workbench/api.py`) is authenticated by the
  trusted `actor` dependency, project+run scoped, GET-only, and fails closed with
  503 until a `run_context_store` is injected (it stays `None` in `create_app`,
  preserving the not-wired-live gate). The `untrusted` PRD/task channel is
  re-scrubbed on the last hop with `scrub_config_payload` (mirroring the
  system-health router), so a credential/host-path/DB-URL/PEM/JWT/AKIA seeded
  into PRD/task prose cannot render; the `trusted` structure (closed typed
  identifiers/digests/enums) is passed through untouched.
- **Security:** the untrusted-prose scrub is defense-in-depth at BOTH
  construction time (`redact_config_text` via `_rc_untrusted_prose` in
  `models.py`) and the API boundary. The proven-leak corpus (AKIA-no-separator,
  JWT, PEM, `ip:port`, `postgres://` URL, `/var/anvil/state.db`) is asserted
  scrubbed from every untrusted field in
  `tests/test_security_contract.py::test_run_context_untrusted_prose_scrubs_full_proven_leak_corpus`.
  A gate override may only STRENGTHEN the default (never weaken it below the
  reviewed floor), proven by
  `tests/test_harness_kernel.py::test_gate_override_may_strengthen_but_never_weaken_the_default`.
- **Schema-version note:** `RunContext.as_dict()` stamps the DISTINCT internal
  identifier `workbench-run-context-internal/v1` — deliberately not the published
  flat contract const `workbench-run-context/v1`
  (`docs/contracts/schemas/run-context.v1.schema.json`), because the trusted/
  untrusted split is the accepted internal shape and does not validate against
  the flat schema. The two shapes are intentionally non-substitutable.
- **Verification:** `tests/test_harness_kernel.py`,
  `tests/test_run_context_integration.py`,
  `tests/test_run_context_adversarial.py`, `tests/test_api.py`,
  `tests/test_security_contract.py`, `tests/test_operation_catalog_integration.py`
  and the full `python -m pytest -q` suite.
