...
def test_stream_responses_keeps_advanced_instructions_and_strips_route_id(monkeypatch):
    # Advanced mode's build_advanced_request emits ``instructions`` (R003: the
    # system prompt, kept separate from ``input``) -- a legitimate Serving field
    # the router accepts.  The allowlist must let it through while still stripping
    # the internal ``route_id``; a too-narrow allowlist would silently swallow the
    # system prompt (the exact bug class R003 guards against).
    body = b"data: {\"type\":\"response.completed\"}\n\ndata: [DONE]\n\n"
    captured = _patch_urlopen(monkeypatch, body)
    request = {
        "model": "chat",
        "route_id": "route.chat",  # internal-only; Serving rejects it
        "input": "hi",
        "instructions": "Be terse.",  # R003 system prompt; Serving accepts it
        "stream": True,
        "max_output_tokens": 16,
    }
    list(stream_responses(ROUTER_BASE_URL, ROUTER_TOKEN, request, _FakeCancel()))
    sent = json.loads(captured["body"].decode("utf-8"))
    assert "route_id" not in sent
    assert sent["instructions"] == "Be terse."  # system prompt must survive
    assert sent == {
        "model": "chat",
        "input": "hi",
